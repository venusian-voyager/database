...
class ModelInspector
{
     ...
     *
     * @param  class-string<\Voyager\Database\Instrument\Model>|string  $model
    ...
    public function inspect($model, $connection = null)
    {
        ...
        /** @var \Voyager\Database\Instrument\Model $model */
     ...
     * @return \Voyager\NutsAndBolts\Collection<int, array<string, mixed>>
     */
    ...
    {
        // Laravel reports the model's policy here. Auth is out of scope for
        // this port, so there is no Gate to ask.
        $policy = null;
    ...
    protected function getObservers($model)
    {
        ...
        $listeners = array_filter($listeners, function ($v, $key) use ($model) {
            return Str::startsWith($key, 'instrument.') && Str::endsWith($key, $model::class);
        }, ARRAY_FILTER_USE_BOTH);

        // Format listeners Instrument verb => Observer methods...
        $extractVerb = function ($key) {
            preg_match('/instrument.([a-zA-Z]+)\: /', $key, $matches);
     ...
     *
     * @param  \Voyager\Database\Instrument\Model  $model
     * @return class-string<\Voyager\Database\Instrument\Collection>
     */
     ...
     *
     * @param  TModel  $model
     * @return class-string<\Voyager\Database\Instrument\Builder<TModel>>
     */
     ...
     *
     * @param  \Voyager\Database\Instrument\Model  $model
     * @return \Voyager\Http\Resources\Json\JsonResource|null
     */
     ...
     *
     * @param  string  $model
     * @return class-string<\Voyager\Database\Instrument\Model>
     *
     * @see \Voyager\Console\GeneratorCommand
     */

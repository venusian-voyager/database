...
trait HasEvents
{
     ...
     *
     * Allows for object-based events for native Instrument events.
     *
    ...
    public static function resolveObserveAttributes()
    {
        ...
        $isInstrumentGrandchild = is_subclass_of(static::class, Model::class)
            && get_parent_class(static::class) !== Model::class;

        return (new Collection($reflectionClass->getAttributes(ObservedBy::class)))
            ->map(fn ($attribute) => $attribute->getArguments())
            ->flatten()
            ->when($isInstrumentGrandchild, function (Collection $attributes) {
    ...
    protected static function registerModelEvent($event, $callback)
    {
        if (isset(static::$dispatcher)) {
            ...
            static::$dispatcher->listen("instrument.{$event}: {$name}", $callback);
        }
    }
    ...
    protected function fireModelEvent($event, $halt = true)
    {
        ...
        return ! empty($result) ? $result : static::$dispatcher->{$method}(
            "instrument.{$event}: ".static::class, $this
        );
    }
    ...
    public static function flushEventListeners()
    {
        ...
        foreach ($instance->getObservableEvents() as $event) {
            static::$dispatcher->forget("instrument.{$event}: ".static::class);
        }
